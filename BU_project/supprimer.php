              ...
              <div id="tab1">
                ...
                $reponse = $bdd->query('SELECT * FROM `etudiants`ORDER BY `id_etudiant` ASC');
                    ...
                    while($donnees = $reponse->fetch())
                    {
                          ...
                          echo '</td> <td>'.$donnees['nom'].'</td> <td>'.$donnees['prenom'].' </td><td>'.$donnees['institut'].' </td><td>'.$donnees['specialite'];
                          echo '<form name="FormAjout" action="extd_supprim.php" method="post">';
                             echo '<td>
                                  <label class="tm-w-50">
                                    <input
                                      type="checkbox"
