...
// Configuration de la connexion a la base de donnees
    ...
    $id_bu_impression = $_POST['id_bu'];
    $date_impression = $_POST['date'];

    $reponse = $bdd->query("SELECT * FROM `etudiants` WHERE `contact_etudiant` LIKE '$mail'");
                    if($donnees = $reponse->fetch())
                    {
                        $id_etudiant_impression = $donnees['id_etudiant'];
                        $req = $bdd->prepare('INSERT INTO `impression` (`id_bu_impression`,`id_etudiant_impression`,`date_impression`) 
                         VALUES (:id_bu_impression,:id_etudiant_impression,:date_impression)');
                        $req->execute(array('id_bu_impression'=>isset($_POST['id_bu']) ? $_POST['id_bu'] : '',
                                            'id_etudiant_impression'=>$id_etudiant_impression,
                                            'date_impression'=>isset($_POST['date']) ? $_POST['date'] : ''
                                            ));
                        $req->closeCursor();
                    }
                    else{
          ...
          <div class="tm-page-col-right">
            <?php
                if ($_SESSION['error']!='1')
                {
                    ...
                    Impression envoyer</h3>';
                }
                else{
                    echo '<h3 class="tm-text-secondary tm-mb-5">
                    etudiant introvable Verifier bien que vous avez un compte !!</h3>';
                }
              ?>
